...
void (*_amod_mouse_capture)(int onoff)=NULL;
...
int amod_init(void) {
    ...
    if ((tmp=GetProcAddress(dll_instance,"amod_mouse_capture"))) _amod_mouse_capture=tmp;
...
void amod_mouse_capture(int onoff) {
    if (_amod_mouse_capture) _amod_mouse_capture(onoff);
}
